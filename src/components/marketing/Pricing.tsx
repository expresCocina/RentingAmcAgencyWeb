...
const plans = [
  {
    name: "Digital Core",
    price: { monthly: 490000, annual: 392000 },
    description: "Presencia profesional y autoridad de marca.",
    badge: "Desde $490.000/mes",
    features: ["Desarrollo Web Next.js 15", "Hosting & Mantenimiento Pro", "SEO Tecnico & Indexacion Google", "Gestion Social (2 redes)", "Soporte 24/7"],
    icon: <Globe className="text-blue-400" size={24} />,
    color: "from-blue-500/20 to-cyan-500/20"
  },
  {
    name: "Growth & Ads",
    price: { monthly: 990000, annual: 792000 },
    description: "Escalabilidad agresiva y captura de leads.",
    highlight: true,
    features: ["Todo en Digital Core", "Gestion de Meta & Google Ads", "Estrategia de Funnels de Venta", "Creacion de Contenido Viral", "Integracion de CRM Vida Digital", "Dashboard de Analitica en Vivo"],
    icon: <TrendingUp className="text-blue-500" size={24} />,
    color: "from-blue-600/30 to-indigo-600/30"
  },
  {
    name: "Elite Partner",
    price: { monthly: 1490000, annual: 1192000 },
    description: "Departamento digital externo completo.",
    features: ["Todo en Growth & Ads", "Inbound Marketing & Emailing", "Automatizacion con IA", "Branding & Identidad Visual", "Software a Medida Ilimitado", "Consultoria Estrategica Semanal"],
...
export const Pricing = () => {
  ...
  async function handleCheckout(formData: FormData) {
    ...
    if (result.success) {
      setFeedback({ type: "success", text: "Aplicacion prioritaria recibida! Nos comunicaremos contigo en breve." });
      setTimeout(() => { setSelectedPlan(null); setFeedback(null); }, 3000);
    } else {
    ...
    <section id="renting" className="py-32 px-6 relative overflow-hidden bg-black">
      <div className="max-w-7xl mx-auto relative z-10">
        <Reveal>
          <div className="text-center mb-16">
            <h2 className="text-3xl sm:text-4xl md:text-6xl lg:text-7xl font-black tracking-tighter italic mb-6">
              PLANES Y <span className="text-blue-500">PRECIOS</span>
            </h2>
            <p className="text-gray-400 max-w-2xl mx-auto font-light italic">
              Elige el plan que se adapta a tu negocio. Sin permanencia forzada, sin costos ocultos, cancelas cuando quieras.
            </p>
          </div>
        </Reveal>
        ...
        <div className="flex justify-center items-center gap-4 mb-16">
          ...
          <button onClick={() => setIsAnnual(!isAnnual)} className="w-16 h-8 rounded-full bg-white/5 border border-white/10 p-1 flex items-center transition-colors">
            ...
            <Reveal key={i} width="100%">
              <div className={`relative p-1 bg-[#0a0a0a] rounded-[40px] border ${plan.highlight ? "border-blue-500" : "border-white/10"} h-full group hover:scale-[1.02] transition-transform duration-500`}>
                ...
                <div className="p-8 md:p-10 h-full flex flex-col relative z-10">
                  ...
                  {plan.badge && !isAnnual && (
                    <span className="inline-block mb-3 text-[10px] font-black tracking-widest uppercase px-3 py-1 rounded-full bg-blue-500/10 border border-blue-500/20 text-blue-400 w-fit">
                      {plan.badge}
                    </span>
                  )}
                  ...
                  <div className="mb-8">
                    <div className="flex items-baseline gap-1">
                      <span className="text-4xl md:text-5xl font-black">
                        {formatCOP(isAnnual ? plan.price.annual : plan.price.monthly)}
                      </span>
                    </div>
      ...
      <AnimatePresence>
        {selectedPlan && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 md:p-6">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setSelectedPlan(null)} className="absolute inset-0 bg-black/80 backdrop-blur-md cursor-pointer" />
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="relative w-full max-w-lg bg-[#050505] border border-blue-500/30 rounded-[40px] shadow-[0_0_80px_rgba(59,130,246,0.15)] overflow-hidden">
              <div className="p-8 md:p-10 relative">
                <div className="absolute top-0 left-1/2 -translate-x-1/2 bg-blue-600/20 border-b border-x border-blue-500/30 px-6 py-1 rounded-b-xl">
                  <span className="text-blue-400 text-[9px] font-black uppercase tracking-widest">Solo 3 cupos disponibles este mes</span>
                </div>
                <button onClick={() => setSelectedPlan(null)} className="absolute top-6 right-6 text-gray-500 hover:text-white transition-colors z-10"><X size={24} /></button>
                <div className="mb-8 mt-4">
                  <p className="text-gray-500 text-xs font-bold tracking-widest uppercase mb-2">Aplicacion Estrategica</p>
                  ...
                  <p className="text-gray-400 font-light mt-3 text-sm leading-relaxed">
                    Cuentanos sobre tu negocio y evaluaremos si tenemos la infraestructura para multiplicar tu facturacion.
                  </p>
                </div>
                <form action={handleCheckout} className="space-y-5">
                  <input type="hidden" name="service" value={`[VIP APP] Plan: ${selectedPlan.name} (${isAnnual ? "Anual" : "Mensual"}) - ${formatCOP(isAnnual ? selectedPlan.price.annual : selectedPlan.price.monthly)}/mes`} />
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-bold tracking-widest uppercase text-blue-500/70 ml-4 block">Nombre del CEO / Fundador</label>
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-bold tracking-widest uppercase text-blue-500/70 ml-4 block">Email Corporativo</label>
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-bold tracking-widest uppercase text-blue-500/70 ml-4 block">WhatsApp + Mayor Desafio Actual</label>
                  ...
                  <motion.button disabled={isPending} whileHover={{ scale: isPending ? 1 : 1.02 }} whileTap={{ scale: isPending ? 1 : 0.98 }} className="w-full mt-6 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white font-black py-5 rounded-2xl flex items-center justify-center gap-3 disabled:opacity-50 transition-all shadow-[0_0_30px_rgba(59,130,246,0.3)] uppercase tracking-widest text-xs">
                    {isPending ? <><Loader2 className="animate-spin" size={20} /> Evaluando Perfil...</> : <><TrendingUp size={18} /> Enviar Aplicacion VIP</>}
                  </motion.button>
                  <p className="text-center text-[10px] text-gray-500 uppercase tracking-widest mt-4 flex items-center justify-center gap-2">
                    <Check size={12} className="text-green-500" /> Tus datos estan encriptados
                  </p>
                  {feedback && (
