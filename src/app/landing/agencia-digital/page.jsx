...
// ─── Helper: fire FB Pixel event (navegador) ───────────────────────
function fbTrackLead() {
  try {
    if (typeof window !== "undefined" && window.fbq) {
      window.fbq("track", "Lead", { content_name: "Landing Agencia Digital", currency: "COP", value: 0 });
...
// ══════════════════════════════════════════════════════════════════
function LeadModal({ open, onClose, waMsg, service }) {
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    if (!name.trim() || !phone.trim()) { setError("Por favor completa nombre y WhatsApp."); return; }
    ...
    try {
      await fetch("/api/leads", {
        ...
        body: JSON.stringify({
          ...
          service: service || "Landing Agencia Digital",
          source: "landing_agencia_digital",
        }),
      });
      fbTrackLead();
    } catch (_) {}
    setLoading(false);
    const personalMsg = `${waMsg}\n\nSoy ${name.trim()} — mi WhatsApp: ${phone.trim()}.`;
    window.open(waLink(personalMsg), "_blank");
    onClose();
  }
      ...
      {open && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <motion.div
            ...
            className="relative w-full max-w-md bg-[#0f0f0f] border border-yellow-500/20 rounded-[32px] shadow-[0_0_80px_rgba(234,179,8,0.12)] overflow-hidden"
          >
            <div className="px-8 pt-8 pb-4">
              <button onClick={onClose} className="absolute top-5 right-6 text-gray-600 hover:text-white transition-colors text-xl font-black">✕</button>
              <span className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-green-500/10 border border-green-500/20 text-green-400 text-[10px] font-black tracking-[0.2em] uppercase mb-3">
                <span className="w-1.5 h-1.5 rounded-full bg-green-400 animate-pulse" /> Un paso antes de WhatsApp
              </span>
              <h3 className="text-2xl font-black tracking-tighter text-white leading-tight">¿Con quién hablamos?</h3>
              <p className="text-gray-400 text-sm mt-2 leading-relaxed">
                Déjanos tu nombre y WhatsApp — te contactamos en menos de 10 minutos.
              </p>
            </div>
            <form onSubmit={handleSubmit} className="px-8 pb-8 pt-3 space-y-3">
              <div>
                <label className="text-[10px] font-black tracking-widest uppercase text-yellow-500/70 block mb-1 ml-1">Nombre completo *</label>
                <input ref={inputRef} value={name} onChange={e => setName(e.target.value)} required type="text"
                  placeholder="Ej. Carlos Martínez"
                  className="w-full bg-white/5 border border-white/10 rounded-2xl px-4 py-3.5 text-white placeholder:text-gray-600 focus:border-yellow-500/50 focus:bg-yellow-500/5 outline-none transition-all text-sm" />
              </div>
              <div>
                <label className="text-[10px] font-black tracking-widest uppercase text-yellow-500/70 block mb-1 ml-1">WhatsApp *</label>
                <input value={phone} onChange={e => setPhone(e.target.value)} required type="tel"
                  placeholder="+57 300 000 0000"
                  className="w-full bg-white/5 border border-white/10 rounded-2xl px-4 py-3.5 text-white placeholder:text-gray-600 focus:border-yellow-500/50 focus:bg-yellow-500/5 outline-none transition-all text-sm" />
              </div>
              <div>
                <label className="text-[10px] font-black tracking-widest uppercase text-yellow-500/70 block mb-1 ml-1">
                  Email <span className="text-gray-600 font-normal normal-case tracking-normal">(opcional)</span>
                </label>
              ...
              <motion.button type="submit" disabled={loading}
                ...
                className="w-full mt-2 flex items-center justify-center gap-3 bg-[#25D366] hover:bg-[#1ebe5d] disabled:opacity-60 text-white font-black py-4 rounded-2xl transition-colors shadow-lg shadow-green-500/20 text-sm"
              >
                {loading ? (
                  <><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> Guardando...</>
                ) : (
                  <><WaIcon /> Continuar a WhatsApp →</>
                )}
              </motion.button>
              <p className="text-center text-[10px] text-gray-600 uppercase tracking-widest">🔒 Tus datos son 100% privados</p>
            </form>
          </motion.div>
        </div>
      )}
...
function WaIcon({ size = 20 }) {
  return (
    <svg width={size} height={size} className="fill-white flex-shrink-0" viewBox="0 0 24 24">
      ...
      <div className="absolute inset-0 bg-white overflow-hidden">
        {!blocked ? (
          <>
            ...
            <iframe src={p.link} title={p.title} loading="lazy" sandbox="allow-scripts allow-same-origin"
              onLoad={() => setLoaded(true)} onError={() => setBlocked(true)}
              className="absolute top-0 left-0 border-0"
              style={{ width: "300%", height: "300%", transform: "scale(0.333)", transformOrigin: "top left",
                pointerEvents: "none", opacity: loaded ? 1 : 0, transition: "opacity 0.5s ease" }} />
          </>
        ) : (
  ...
  useEffect(() => {
    ...
    const obs = new IntersectionObserver(([entry]) => {
      if (entry.isIntersecting && !started.current) {
        ...
        const isDecimal = target % 1 !== 0; let start = 0;
        const step = target / (duration / 16);
        const timer = setInterval(() => {
          start = Math.min(start + step, target);
          setDisplay((isDecimal ? start.toFixed(1) : Math.round(start)) + suffix);
...
const services = [
  {
    id: "ads", name: "Publicidad Digital", emoji: "📣",
    accent: "from-pink-500 to-rose-500", accentBg: "bg-pink-500/10",
    accentBorder: "border-pink-500/30", accentText: "text-pink-400", accentRing: "ring-pink-500/50",
    plans: [
      { name: "STARTER", price: "$350.000/mes", popular: false, desc: "Para negocios que inician en publicidad digital",
        ...
        waMsg: "Hola, me interesa el plan STARTER de Publicidad Digital" },
      { name: "GROWTH", price: "$650.000/mes", popular: true, desc: "Para negocios que quieren escalar con ambas plataformas",
        ...
        waMsg: "Hola, me interesa el plan GROWTH de Publicidad Digital" },
      { name: "SCALE", price: "Personalizado", popular: false, desc: "Para marcas con altos volúmenes de inversión",
        ...
        waMsg: "Hola, me interesa el plan SCALE de Publicidad Digital" },
    ],
  },
  {
    id: "seo", name: "SEO & Posicionamiento", emoji: "🔍",
    accent: "from-lime-400 to-green-500", accentBg: "bg-lime-500/10",
    accentBorder: "border-lime-500/30", accentText: "text-lime-400", accentRing: "ring-lime-500/50",
    plans: [
      { name: "LOCAL SEO", price: "$280.000/mes", popular: false, desc: "Para negocios locales y pymes",
        features: ["Auditoría SEO inicial", "5 keywords objetivo", "Optimización On-Page", "Google My Business", "Reporte mensual"],
        waMsg: "Hola, me interesa el plan LOCAL SEO" },
      { name: "GROWTH SEO", price: "$550.000/mes", popular: true, desc: "Para empresas que quieren liderar su sector",
        ...
        waMsg: "Hola, me interesa el plan GROWTH SEO" },
      { name: "ENTERPRISE", price: "Personalizado", popular: false, desc: "Para marcas con alta competencia digital",
        ...
        waMsg: "Hola, me interesa el plan ENTERPRISE de SEO" },
    ],
  },
  {
    id: "social", name: "Redes Sociales", emoji: "📱",
    accent: "from-purple-500 to-fuchsia-500", accentBg: "bg-purple-500/10",
    accentBorder: "border-purple-500/30", accentText: "text-purple-400", accentRing: "ring-purple-500/50",
    plans: [
      { name: "STARTER SOCIAL", price: "$320.000/mes", popular: false, desc: "Para negocios que buscan presencia básica",
      ...
      { name: "GROWTH SOCIAL", price: "$580.000/mes", popular: true, desc: "Para marcas que quieren crecer en múltiples redes",
      ...
      { name: "FULL PRESENCE", price: "Personalizado", popular: false, desc: "Para marcas con alta demanda de contenido",
  ...
  {
    id: "email", name: "Email Marketing", emoji: "✉️",
    accent: "from-sky-400 to-blue-600", accentBg: "bg-sky-500/10",
    accentBorder: "border-sky-500/30", accentText: "text-sky-400", accentRing: "ring-sky-500/50",
    plans: [
      { name: "EMAIL STARTER", price: "$250.000/mes", popular: false, desc: "Para negocios que quieren arrancar con email",
        features: ["Hasta 3.000 contactos", "2 campañas/mes", "Template premium", "Reporte de métricas"],
        waMsg: "Hola, me interesa el plan EMAIL STARTER" },
      { name: "EMAIL GROWTH", price: "$480.000/mes", popular: true, desc: "Para negocios que quieren automatizar ventas",
        features: ["Hasta 15.000 contactos", "Campañas ilimitadas", "3 flujos automatizados", "A/B Testing", "Reporte semanal"],
        waMsg: "Hola, me interesa el plan EMAIL GROWTH" },
      { name: "EMAIL SCALE", price: "Personalizado", popular: false, desc: "Para empresas con grandes bases de datos",
        features: ["Contactos ilimitados", "Automatizaciones avanzadas", "Integración con CRM", "Equipo dedicado", "Dashboard en tiempo real"],
        waMsg: "Hola, me interesa el plan EMAIL SCALE" },
    ],
  },
];
...
export default function LandingAgenciaDigital() {
  ...
  const [modal, setModal] = useState({ open: false, waMsg: "", service: "" });
  const openModal = (waMsg, service = "Landing Agencia Digital") => setModal({ open: true, waMsg, service });
  const closeModal = () => setModal(m => ({ ...m, open: false }));

  const MSG_GENERAL = "Hola, quiero más información sobre los servicios de AMC Agency";
  const MSG_LANDING = "Hola, vi su publicidad y quiero información sobre sus servicios digitales";

  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white overflow-x-hidden">

      {/* Modal global */}
      <LeadModal open={modal.open} onClose={closeModal} waMsg={modal.waMsg} service={modal.service} />
        ...
        <div className="relative max-w-3xl mx-auto text-center z-10">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
            ...
            <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-black tracking-tighter leading-[1.05] mb-6">
              ¿Tu negocio necesita{" "}
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 to-orange-400">más clientes?</span>
              <br />Nosotros te los{" "}
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 to-orange-400">conseguimos.</span>
            </h1>
            <p className="text-gray-300 text-lg md:text-xl leading-relaxed max-w-2xl mx-auto mb-10 font-light">
              ...
              Páginas web, publicidad, SEO y redes sociales desde <strong className="text-white">$250.000/mes.</strong>
            </p>
            <div className="flex flex-col items-center gap-3">
              <motion.button
                onClick={() => openModal(MSG_LANDING)}
              ...
              >
                <WaIcon size={24} /> Quiero más clientes → Hablar ahora
              </motion.button>
      ...
      <section className="py-16 px-4 bg-black/40 border-y border-white/5">
        <div className="max-w-4xl mx-auto">
          <p className="text-center text-gray-500 text-xs font-black tracking-[0.3em] uppercase mb-10">Números que hablan por nosotros</p>
        ...
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-14">
            <h2 className="text-3xl md:text-5xl font-black tracking-tighter mb-4">
              Negocios reales.{" "}
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 to-orange-400">Resultados reales.</span>
            </h2>
            <p className="text-gray-400 max-w-xl mx-auto">Estos son algunos proyectos que hemos construido para negocios colombianos como el tuyo.</p>
          </div>
      ...
      {/* ═══ SECCIÓN 4 — SERVICIOS Y PRECIOS ═══ */}
      <section className="py-24 px-4 bg-black/40">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-14">
            <h2 className="text-3xl md:text-5xl font-black tracking-tighter mb-4">
              Elige lo que tu{" "}
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 to-orange-400">negocio necesita</span>
            </h2>
            <p className="text-gray-400 max-w-xl mx-auto">Sin permanencia forzada. Sin costos ocultos. Cancelás cuando quieras.</p>
          </div>
          ...
          <AnimatePresence mode="wait">
            <motion.div key={activeService} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.3 }}
              className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {current.plans.map((plan, i) => (
                <div key={i} className={`relative p-7 rounded-3xl border ${current.accentBorder} ${current.accentBg} ${plan.popular ? `ring-2 ${current.accentRing}` : ""}`}>
                  {plan.popular && (
                  ...
                  <button
                    onClick={() => openModal(plan.waMsg, `${current.name} — ${plan.name}`)}
        ...
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-14">
            <h2 className="text-3xl md:text-5xl font-black tracking-tighter">
              ¿Por qué{" "}
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 to-orange-400">AMC Agency</span>{" "}
              y no otra agencia?
            </h2>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
      ...
      <section className="py-24 px-4 bg-black/40">
        <div className="max-w-5xl mx-auto">
          <h2 className="text-3xl md:text-5xl font-black tracking-tighter text-center mb-14">
            Lo que dicen{" "}
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 to-orange-400">nuestros clientes</span>
          </h2>
            ...
            {testimonials.map((t, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
                ...
                <div className="flex items-center gap-3">
                  <div className={`w-11 h-11 rounded-full ${t.color} flex items-center justify-center text-white font-black text-lg flex-shrink-0`}>{t.initial}</div>
                  <div>
      ...
      <section className="py-28 px-4 relative overflow-hidden">
        ...
        <div className="relative max-w-2xl mx-auto text-center z-10">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }}>
            ...
            >
              <WaIcon size={28} /> 💬 Escribir al WhatsApp ahora
            </motion.button>
            <p className="mt-6 text-gray-500 text-xs tracking-widest uppercase">🔒 Sin compromiso · Sin contratos forzados · Respondemos al instante</p>
          </motion.div>
        </div>
      </section>
      ...
      <footer className="py-8 px-4 border-t border-white/5 text-center">
        <p className="text-white font-black text-xl tracking-tighter mb-1">AMC<span className="text-yellow-400">.</span></p>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 md:hidden">
        <button
          onClick={() => openModal(MSG_GENERAL)}
          className="w-full flex items-center justify-center gap-3 bg-[#25D366] hover:bg-[#1ebe5d] text-white font-black py-4 px-6 text-base transition-colors shadow-2xl shadow-green-500/40"
        >
          <WaIcon size={20} /> 💬 Hablar con un asesor ahora →
        </button>
      </div>
